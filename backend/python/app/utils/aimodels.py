...
def get_embedding_model(
    provider: str, config: Dict[str, Any], model_name: str | None = None
) -> Embeddings:
    configuration = config["configuration"]
    is_default = config.get("isDefault")
    if is_default and model_name is None:
        model_names = [
            name.strip() for name in configuration["model"].split(",") if name.strip()
        ]
        ...
        if model_name not in model_names:
            raise ValueError(
                f"Model name {model_name} not found in {configuration['model']}"
            )
    ...
    if provider == EmbeddingProvider.AZURE_OPENAI.value:
        ...
        return OllamaEmbeddings(model=model_name, base_url=configuration["endpoint"])
    ...
    if provider == LLMProvider.ANTHROPIC.value:
        ...
        return ChatOllama(
            ...
            base_url=configuration.get(
                "endpoint", os.getenv("OLLAMA_API_URL", "http://localhost:11434")
            ),
        )
